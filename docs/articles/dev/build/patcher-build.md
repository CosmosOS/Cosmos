...
`Cosmos.Patcher.Build` wires the [`Cosmos.Patcher`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher) tool into the MSBuild pipeline. `Cosmos.Patcher` rewrites IL in candidate assemblies by applying CosmosOS-style plugs so that the .NET Core framework can run on NativeAOT targets. Static analysis is provided by [`Cosmos.Patcher.Analyzer`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Build.Analyzer.Patcher), which reports plug errors during C# compilation before any IL is rewritten.
...
2. **FindPluggedAssembliesTask** uses [`PlugScanner.FindPluggedAssemblies`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/PlugScanner.cs) to cross-reference plug targets against the candidate assemblies and produces `AssembliesToPatch`.

3. **RunPatcher** iterates over `AssembliesToPatch` and, for each assembly, the [`PatcherTask`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Build.Patcher/Tasks/PatcherTask.cs) launches the [`Cosmos.Patcher`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher) CLI ([`Program`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/Program.cs) → [`PatchCommand.Execute`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/PatchCommand.cs)). Inside `Execute`:
   - The target assembly is loaded with `AssemblyDefinition.ReadAssembly` and plug assemblies are gathered.
   - [`PlugScanner.LoadPlugs`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/PlugScanner.cs) discovers plug types in those assemblies.
   - [`PlugPatcher.PatchAssembly`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/Patching/PlugPatcher.cs) groups plugs by their `[Plug]` target and patches each target type using:
       - [`MethodResolver`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/Resolution/MethodResolver.cs) matches plug signatures (including constructors and `aThis` parameters) by name and parameter types.
       - [`PlugPatcher.Method`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/Patching/PlugPatcher.Method.cs) swaps or clones IL and strips P/Invoke metadata.
       - [`ILCloner`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/IL/ILCloner.cs) clones instructions, remaps parameters, and fixes branch targets.
       - [`TypeImporter`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/IL/TypeImporter.cs) safely imports type/method/field references, fixing self-references that would cause invalid IL metadata.
       - [`PlugPatcher.Property`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/Patching/PlugPatcher.Property.cs) wires getters and setters to plug implementations.
       - [`PlugPatcher.Field`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Patcher/Patching/PlugPatcher.Field.cs) copies constant values and redirects field accesses.
...
5. [`Cosmos.Patcher.Analyzer`](https://github.com/CosmosOS/Cosmos/blob/gen3/src/Cosmos.Build.Analyzer.Patcher) validates plug rules during compilation to catch errors before patching.
