...
namespace Orvid.Compression.Streams
{
    ...
    /// </summary>
    sealed internal class LzwConstants
    {
        /// <summary>
    ...
    public class LzwInputStream : Stream
    {
        ...
        MainLoop: do
            {
                ...
                while (lBitPos < bitIn)
                {
                    ...
                    {
                        if (code >= 256)
                            throw new Exception("corrupt input: " + code + " > 255");
                    ...
                    if (code >= lFreeEnt)
                    {
                        if (code > lFreeEnt)
                        {
                            throw new Exception("corrupt input: code=" + code + ", freeEnt=" + lFreeEnt);
                        }
        ...
        private void ParseHeader()
        {
            ...
            if (result < 0)
                throw new Exception("Failed to read LZW header");

            if (hdr[0] != (LzwConstants.MAGIC >> 8) || hdr[1] != (LzwConstants.MAGIC & 0xff))
            {
                throw new Exception(String.Format("Wrong LZW header. Magic bytes don't match. 0x{0:x2} 0x{1:x2}", hdr[0], hdr[1]));
            }
            ...
            {
                throw new Exception("Stream compressed with " + maxBits + " bits, but decompression can only handle " + LzwConstants.MAX_BITS + " bits.");
            }

            if ((hdr[2] & LzwConstants.RESERVED_MASK) > 0)
            {
                throw new Exception("Unsupported bits set in the header.");
            }
