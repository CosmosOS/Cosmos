...
namespace Cosmos.VS.Package {
  public partial class ConfigurationBase : CustomPropertyPage {
      ...
      if (comboConfiguration.SelectedIndex > -1) {
        if (comboConfiguration.SelectedIndex != ConfigurationBase.CurrentConfigurationIndex) {
          ...
          foreach (CustomPropertyPage page in propPages) {
            if (page.IsDirty == true) { hasUnsavedChanges = true; break; }
          }
          ...
          if (hasUnsavedChanges == true) {
            ...
            if (result == DialogResult.Yes) {
              foreach (CustomPropertyPage page in propPages) { page.ApplyChanges(); }
              hasUnsavedChanges = false;

            } else if (result == DialogResult.No) {
              foreach (CustomPropertyPage page in propPages) { page.IsDirty = false; }
              hasUnsavedChanges = false;

            } else { comboConfiguration.SelectedIndex = ConfigurationBase.CurrentConfigurationIndex; }
          ...
          if (hasUnsavedChanges == false) {
            ...
            CosmosPage.CurrentBuildTarget = EnumValue.Parse(GetConfigProperty("BuildTarget"), BuildTarget.VMware);
    ...
    protected override void FillConfigurations() {
      ...
      foreach (ProjectConfig config in base.ProjectConfigs) { comboConfiguration.Items.Add(config.ConfigName); }

      if (comboConfiguration.SelectedIndex != ConfigurationBase.CurrentConfigurationIndex) { comboConfiguration.SelectedIndex = ConfigurationBase.CurrentConfigurationIndex; }
