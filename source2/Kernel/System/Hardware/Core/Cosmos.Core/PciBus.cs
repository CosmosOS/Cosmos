...
namespace Cosmos.Core {
  public class PciBus {
    ...
    protected UInt32 GetAddressBase(byte aBus, byte aSlot, byte aFunction) {
      ...
      return (UInt32)(
        ...
        // Bits 10-8
          | (((UInt32)aFunction & 0x07) << 8));
    }

    protected UInt32 ReadRegister(UInt32 aAddrBase, byte aRegister) {
      UInt32 xAddr = aAddrBase | ((UInt32)(aRegister & 0xFC));
    ...
    protected void DebugSendBAR(UInt32 aAddr, byte aBar) {
      ...
      if ((x & 0x01) == 0x01) {
        x = x & 0xFFFFFFFC;
        Global.Dbg.Send("BAR " + aBar + " (IO): " + x.ToHex());
      } else {
    ...
    protected void EnumerateBusses() {
      ...
      for (byte xBus = 0; xBus < 4; xBus++) {
        ...
        for (byte xSlot = 0; xSlot < 32; xSlot++) {
          ...
          // Hopefully there would never be a gap in functions like there are devices
          // but you never know.... So we scan them all.
