...
[DebuggerStepThrough]
public static unsafe class GCImplementation
{
    private static byte* memPtr = null;
    private static ulong memLength;
    private static bool StartedMemoryManager;

    /// <summary>
    ///     Acquire lock. Not implemented.
    /// </summary>
    /// <exception cref="NotImplementedException">Thrown always.</exception>
    private static void AcquireLock() => throw new NotImplementedException();

    /// <summary>
    ///     Release lock. Not implemented.
    /// </summary>
    /// <exception cref="NotImplementedException">Thrown always.</exception>
    private static void ReleaseLock() => throw new NotImplementedException();

    /// <summary>
    ///     Alloc new object.
    /// </summary>
    public static uint AllocNewObject(uint aSize) => (uint)Heap.Alloc(aSize);

    /// <summary>
    ///     Free Object from Memory
    /// </summary>
    /// <param name="obj">Takes a memory allocated object</param>
    public static void Free(object aObj) => Heap.Free(GetPointer(aObj));

    /// <summary>
    ///     Get amount of available Ram
    /// </summary>
    /// <returns>Returns amount of available memory to the System in MB</returns>
    public static ulong GetAvailableRAM() => memLength / 1024 / 1024;

    /// <summary>
    ///     Get a rough estimate of used Memory by the System
    /// </summary>
    /// <returns>Returns the used PageSize by the MemoryManager in Bytes.</returns>
    public static uint GetUsedRAM() => (RAT.TotalPageCount - RAT.GetPageCount()) * RAT.PageSize;

    /// <summary>
    ///     Initialise the Memory Manager, this should not be called anymore since it is done very early during the boot
    ///     process.
    /// </summary>
    ...
    public static uint* GetPointer(object aObj) => throw null; // this is plugged
    ...
    /// <returns></returns>
    public static uint GetSafePointer(object aObj) => (uint)GetPointer(aObj);
    ...
    /// <param name="aObj"></param>
    /// <returns></returns>
    public static uint GetType(object aObj) => *GetPointer(aObj);
    ...
    public static void IncRootCount(ushort* aPtr)
    {
        if (RAT.GetPageType(aPtr) != 0)
        {
            ...
            *(aPtr - 1) =
                (ushort)((rootCount + 1) <<
                         1); // loest bit can be zero since we shouldnt be doing this while gc is collecting
        }
    }
    ...
    {
        var count = VTablesImpl.GetGCFieldCount(aType);
        var offset = VTablesImpl.GetGCFieldOffsets(aType);
