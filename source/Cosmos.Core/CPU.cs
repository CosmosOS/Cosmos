...
public static class CPU
{
    ...
    /// </summary>
    public static uint GetAmountOfRAM() => Multiboot2.GetMemUpper() / 1024;
    ...
    public static long EstimateCPUSpeedFromName(string s)
    {
        ...
        var w = new string[words.Length];
        for (var i = 0; i < words.Length; i++) // Switch order
        {
        ...
        for (var i = 0; i < words.Length; i++)
        {
            ...
            if (value == 0)
            {
                if (Double.TryParse(word, out value) ||
                    Double.TryParse(word.Substring(0, word.Length - 3), out value))
                {
                    break;
                }
            }
    ...
    /// </summary>
    /// <returns>
    ///     Returns an array of MemoryMaps containing the Multiboot Memory Map information. The array may have empty
    ///     values at the end.
    /// </returns>
    public static unsafe MemoryMapBlock[] GetMemoryMap()
    {
        ...
        var totalSize = Multiboot2.MemoryMap->Size - 16;
        var entrySize = Multiboot2.MemoryMap->EntrySize;
...
public struct RawMemoryMapBlock
{
    ...
    /// </summary>
    [FieldOffset(8)] public ulong Length;
    ...
    ///     Zero
    /// </summary>
    [FieldOffset(20)] public uint Zero;
}
