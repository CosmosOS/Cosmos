...
public static unsafe class Heap
{
    ...
    public static byte* Alloc(uint aSize)
    {
        ...
        {
            var ptr = HeapSmall.Alloc((ushort)aSize);
            CPU.EnableInterrupts();
            return ptr;
        }

        if (aSize <= HeapMedium.MaxItemSize)
        {
            var ptr = HeapMedium.Alloc(aSize);
            CPU.EnableInterrupts();
            return ptr;
        }
        else
        {
            var ptr = HeapLarge.Alloc(aSize);
            CPU.EnableInterrupts();
            return ptr;
        }
    }
    ...
    /// <returns></returns>
    public static uint SafeAlloc(uint aSize) => (uint)Alloc(aSize);
    ...
    public static int Collect()
    {
        ...
        for (var ratIndex = 0; ratIndex < RAT.TotalPageCount; ratIndex++)
        {
        ...
        {
            var size = rootSMTPtr->Size;
            var objectSize = size + HeapSmall.PrefixItemBytes;
            var objectsPerPage = RAT.PageSize / objectSize;
            ...
            while (smtBlock != null)
            {
                ...
                for (var i = 0; i < objectsPerPage; i++)
                {
        ...
        var currentStackPointer = (uint*)CPU.GetEBPValue();
    ...
    public static void SweepTypedObject(uint* obj, uint type)
    {
        ...
        var fields = VTablesImpl.GetGCFieldCount(type);
    ...
    /// </summary>
    /// <returns></returns>
    private static uint GetStringTypeID() => UInt32.MaxValue; // so that tests still pass return bogus value
}
