...
/// </summary>
public static unsafe class HeapSmall
{
    /// <summary>
    ...
    public static void Init()
    {
        ...
        // 4 slots, ~1k ea
        var xMaxItemSize = RAT.PageSize / 4 - PrefixItemBytes;
        // Word align it
    ...
    private static void InitSMT(uint aMaxItemSize)
    {
    ...
    {
        var page = (SMTPage*)RAT.AllocPages(RAT.PageType.SMT);
    ...
    {
        var xPtr = (byte*)RAT.AllocPages(RAT.PageType.HeapSmall);
        if (xPtr == null)
        {
            return; // we failed to create the page, Alloc should still handle this case
        }

        var xSlotSize = aItemSize + PrefixItemBytes;
        var xItemCount = RAT.PageSize / xSlotSize;
        for (uint i = 0; i < xItemCount; i++)
        {
            var xSlotPtr = xPtr + i * xSlotSize;
            var xMetaDataPtr = (ushort*)xSlotPtr;
    ...
    {
        var ptr = GetFirstBlock(page, aSize)->First;
        if (ptr == null)
        {
            return null;
        }
    ...
    private static SMTBlock* GetFirstWithSpace(SMTPage* aPage, uint aSize) =>
        GetFirstWithSpace(aSize, GetFirstBlock(aPage, aSize));
