...
/// </summary>
public static unsafe class RAT
{
    /// <summary>
    ...
    public static void Init(byte* aStartPtr, uint aSize)
    {
        ...
        var xRatPageCount = (TotalPageCount - 1) / PageSize + 1;
        var xRatTotalSize = xRatPageCount * PageSize;
        mRAT = RamStart + RamSize - xRatTotalSize;

        // Mark empty pages as such in the RAT Table
        for (var p = mRAT; p < mRAT + TotalPageCount - xRatPageCount; p++)
        {
            *p = PageType.Empty;
        }

        // Mark the rat pages as such
        for (var p = mRAT + TotalPageCount - xRatPageCount; p < mRAT + xRatTotalSize; p++)
        {
            *p = PageType.RAT;
        }
    ...
    public static uint GetPageCount(byte aType = 0)
    {
        uint xResult = 0;
        var xCounting = false;
        for (var p = mRAT; p < mRAT + TotalPageCount; p++)
        {
            if (*p == aType)
            {
                xResult++;
    ...
    public static void* AllocPages(byte aType, uint aPageCount = 1)
    {
        ...
        else
        {
            ...
            for (var p = mRAT + TotalPageCount - 1; p >= mRAT; p--)
            {
                if (*p == PageType.Empty)
                {
        ...
        {
            var diff = xPos - mRAT;
            var xResult = RamStart + diff * PageSize;
            *xPos = aType;
            for (var p = xPos + 1; p < xPos + xCount; p++)
            {
