...
public class MACAddress : IComparable
{
    ...
    public bool IsValid() => bytes != null && bytes.Length == 6;
    ...
    public override int GetHashCode() => (GetType().AssemblyQualifiedName + "|" + ToString()).GetHashCode();

    public ulong ToNumber() =>
        (ulong)((bytes[0] << 40) | (bytes[1] << 32) | (bytes[2] << 24) | (bytes[3] << 16) |
                (bytes[4] << 8) | (bytes[5] << 0));
    ...
    public uint To32BitNumber() =>
        (uint)((bytes[0] << 40) | (bytes[1] << 32) | (bytes[2] << 24) | (bytes[3] << 16) |
               (bytes[4] << 8) | (bytes[5] << 0));
