...
public class PCIDevice
{
    ...
    protected static Core.IOGroup.PCI IO = new();
    ...
    {
        var xAddr = GetAddressBase(Bus, Slot, Function) | (0xE & 0xFC);
    ...
    {
        var xAddr = GetAddressBase(Bus, Slot, Function) | (0x0 & 0xFC);
        IO.ConfigAddressPort.DWord = xAddr;
    ...
    {
        var command = ReadRegister16(0x04);
    ...
    private static string ToHex(uint aNumber, byte aBits) => "0x" + aNumber.ToHex(aBits / 4);
    ...
    {
        var xAddr = GetAddressBase(bus, slot, function) | (uint)(aRegister & 0xFC);
        IO.ConfigAddressPort.DWord = xAddr;
