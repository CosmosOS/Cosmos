...
public class AHCI
{
    ...
    public string Version => ((byte)mGeneric.AHCIVersion >> 24) + (byte)(mGeneric.AHCIVersion >> 16) + "." +
                             (byte)(mGeneric.AHCIVersion >> 8) +
                             ((byte)mGeneric.AHCIVersion > 0 ? "." + (byte)mGeneric.AHCIVersion : "");

    internal static void InitDriver()
    {
        if (xDevice != null)
        {
            var Driver = new AHCI(xDevice);
        }
    }
    ...
    {
        byte xVoid;
        for (var i = 0; i < microsecondsTimeout; i++)
        {
            // Random IOPort
    ...
    private void GetPorts()
    {
        ...
        for (; xPort < 32; xPort++)
        {
            ...
            {
                var xPortReg = new PortRegisters((uint)mABAR + 0x100, (uint)xPort);
                var PortType = CheckPortType(xPortReg);
                xPortReg.mPortType = PortType;
                var xPortString =
                    "0:" + (xPort.ToString().Length <= 1 ? xPort.ToString().PadLeft(1, '0') : xPort.ToString());
                ...
                {
                    mAHCIDebugger.Send("Port Multiplier Drive at port " + xPortString +
                                       " found, which is not supported yet!");
                }
    ...
    private PortType CheckPortType(PortRegisters aPort)
    {
        ...
        switch ((AHCISignature)xSignature)
        {
            ...
            default:
                throw new Exception("SATA Error: Unknown drive found at port: " + aPort.mPortNumber);
                ;
        }
    }
    ...
    {
        var xCMDHeader = new HBACommandHeader[32];
    ...
    private bool StopCMD(PortRegisters aPort)
    {
        ...
        for (xSpin = 0; xSpin < 101; xSpin++)
        {
            if ((aPort.CMD & (uint)CommandAndStatus.CMDListRunning) == 0 &&
                ...
                (aPort.CMD & (uint)CommandAndStatus.FISRecieveEnable) == 0)
            {
                break;
            }
