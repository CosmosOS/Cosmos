...
{
    internal static Debugger mSATAPIDebugger = new("HAL", "SATAPI");
    ...
    public void SendSATAPICommand(ATACommands aCommand, uint aStart, uint aCount)
    {
        ...
        var xSlot = FindCMDSlot(mPortReg);
        if (xSlot == -1)
        {
            return;
        }
        ...
        for (var i = 0; i < xCMDHeader.PRDTL - 1; i++)
        {
            xCMDTable.PRDTEntry[i].DBA = DataBaseAddress;
            xCMDTable.PRDTEntry[i].DBC =
        ...
        xCMDTable.PRDTEntry[xCMDHeader.PRDTL - 1].DBC =
            aCount * 512 - 1; // 8K bytes (this value should always be set to 1 less than the actual value)
        ...
        {
            new MemoryBlock(xCMDTable.ACMD, 12).Bytes[i] = xATAPICMD[i];
        }
