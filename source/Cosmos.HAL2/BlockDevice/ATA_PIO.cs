...
{
    internal static Debugger mDebugger = new("HAL", "AtaPio");
    ...
    public ATA_PIO(ATA aIO, ControllerIdEnum aControllerId, BusPositionEnum aBusPosition)
    {
        IO = aIO;
    ...
    private static void Debug(string message) => mDebugger.Send("AtaPio debug: " + message);
    ...
    public SpecLevel DiscoverDrive()
    {
        ...
        if ((xIdentifyStatus & Status.DRQ) == 0)
        {
            // Error
            return SpecLevel.Null;
        }
    ...
    protected string GetString(ushort[] aBuffer, int aIndexStart, int aStringLength)
    {
        ...
        var array = new byte[aStringLength];
        var counter = 0;
        for (var i = aIndexStart; i < aIndexStart + aStringLength / 2; i++)
        {
            var item = aBuffer[i];
    ...
    protected void InitDrive()
    {
        ...
        if (LBA48Bit)
        {
            mBlockCount = (((ulong)xBuff[102] << 32) | ((ulong)xBuff[101] << 16) | xBuff[100]) - 1;
        }
    }
    ...
    /// <summary>
    ///     Reads the data from a specific starting block using aBlockCount (how many blocks to be read), and fills the
    ///     specified byte[] with the block.
    /// </summary>
