...
public class ATAPI : BlockDevice
{
    ...
    private readonly ATA_PIO device;
    ...
    public override void ReadBlock(ulong SectorNum, ulong SectorCount, ref byte[] aData)
    {
        ...
        packet[0] = (byte)Cmd.Read; //Read Sector
        packet[1] = 0;
    ...
    private void SendCmd(byte[] AtapiPacket, int size, ref ushort[] outputData)
    {
        ...
        IO.DeviceSelect.Byte = (byte)((byte)(DvcSelVal.Default | DvcSelVal.LBA |
        ...
        //Set commmand size
        IO.LBA1.Byte = SectorSize & 0xFF;
        IO.LBA2.Byte = (SectorSize >> 8) & 0xFF;
        ...
        for (var i = 0; i < AtapiPacket.Length; i++)
        {
            var b1 = AtapiPacket[i];
            var b2 = AtapiPacket[i + 1];

            var toSend = BitConverter.ToUInt16(new[] { b1, b2 }, 0);
            IO.Data.Word = toSend;
    ...
    private void Poll(bool checkDRQ = true)
    {
        ...
        while (true)
        {
            ...
            if (checkDRQ)
            {
                if ((IO.Status.Byte & (byte)Status.Busy) == 0 && (IO.Status.Byte & (byte)Status.DRQ) != 0)
                {
                    break;
                }
            }
            ...
            {
                if ((IO.Status.Byte & (byte)Status.Busy) == 0)
                {
                    break;
                }
            }
    ...
    {
        if ((IO.Status.Byte & (byte)Status.Error) != 0)
        {
            throw new Exception("ATA Error occured!");
        }

        if ((IO.Status.Byte & (byte)Status.ATA_SR_DF) != 0)
        {
            throw new Exception("ATA device fault encountered!");
        }

        if ((IO.Status.Byte & (byte)Status.DRQ) == 0)
        {
