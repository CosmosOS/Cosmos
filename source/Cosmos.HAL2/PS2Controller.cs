...
public class PS2Controller : Device
{
    ...
    private readonly Core.IOGroup.PS2Controller IO = Core.Global.BaseIOGroups.PS2Controller;
    private readonly Debugger mDebugger = new("HAL", "PS2Controller");
    ...
    private void IdentifyDevice(byte aPort, out Device aDevice, bool InitScrollWheel)
    {
        ...
        if (aPort == 1 || aPort == 2)
        {
            ...
            if (aDevice == null)
            {
                ...
                Console.WriteLine("(PS/2 Controller) Device detection failed: FirstByte: " + xFirstByte +
                                  ", SecondByte: " + xSecondByte);
            }
        }
        else
        {
        ...
        if (aPort == 1 || aPort == 2)
        {
            ...
            switch (xTestByte)
            {
                ...
                case 0x04:
                    ...
                    break;
                default:
                    throw new Exception("(PS/2 Controller) (Port " + aPort + ") Device test status unknown: " +
                                        xTestByte.ToHex());
            }
    ...
    /// </summary>
    public void PrepareSecondPortWrite() => SendCommand(Command.WriteNextByteToSecondPS2PortInputBuffer);
    ...
    public bool WaitToReadData()
    {
        ...
        while ((IO.Status.Byte & 1) == 0)
        {
            ...
            if (i >= WAIT_TIMEOUT)
            {
                mDebugger.SendInternal("Timeout expired in PS2Controller.WaitToReadData(), IO.Status.Byte: " +
                                       IO.Status.Byte);
                return false;
            }
        }
