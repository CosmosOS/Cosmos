...
public class VMWareSVGAII
{
    ...
    /// <returns>uint value.</returns>
    protected uint GetFIFO(FIFO cmd) => FIFO_Memory[(uint)cmd];
    ...
    /// <returns></returns>
    protected uint SetFIFO(FIFO cmd, uint value) => FIFO_Memory[(uint)cmd] = value;
    ...
    public void SetPixel(uint x, uint y, uint color) => VideoMemory[(y * width + x) * depth + FrameSize] = color;
    ...
    /// <returns>uint value.</returns>
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    public uint GetPixel(uint x, uint y) => VideoMemory[(y * width + x) * depth];
    ...
    public void Clear(uint color) => VideoMemory.Fill(FrameSize, FrameSize, color);
    ...
    public void Fill(uint x, uint y, uint width, uint height, uint color)
    {
        ...
        else
        {
            if ((capabilities & (uint)Capability.RectCopy) != 0)
            {
                ...
                var xTarget = x + width;
                var yTarget = y + height;

                for (var xTmp = x; xTmp < xTarget; xTmp++)
                {
                ...
                Update(x, y, width, 1);
                for (var yTmp = y + 1; yTmp < yTarget; yTmp++)
                {
            ...
            else
            {
                ...
                for (var xTmp = x; xTmp < xTarget; xTmp++)
                {
                    for (var yTmp = y; yTmp < yTarget; yTmp++)
                    {
    ...
    /// </summary>
    public void Enable() => WriteRegister(Register.Enable, 1);

    /// <summary>
    ///     Disable the SVGA Driver , return to text mode
    /// </summary>
    public void Disable() => WriteRegister(Register.Enable, 0);
