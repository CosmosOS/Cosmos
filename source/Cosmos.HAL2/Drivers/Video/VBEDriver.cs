...
public class VBEDriver
{
    ...
    public VBEDriver(ushort xres, ushort yres, ushort bpp)
    {
        ...
        else if ((videocard = HAL.PCI.GetDevice(VendorID.VirtualBox, DeviceID.VBVGA)) !=
                 null || //VirtualBox Video Adapter PCI Mode
                 (videocard = HAL.PCI.GetDevice(VendorID.Bochs, DeviceID.BGA)) !=
                 null) // Bochs Graphics Adaptor PCI Mode
        {
            Global.mDebugger.SendInternal(
                $"Creating VBE BGA driver with Mode {xres}*{yres}@{bpp}. Framebuffer address=" + videocard.BAR0);
    ...
    public void SetVRAM(uint index, byte value) => lastbuffer[index] = value;
    ...
    {
        var pixel = (lastbuffer[index + 3] << 24) | (lastbuffer[index + 2] << 16) | (lastbuffer[index + 1] << 8) |
                    lastbuffer[index];
    ...
    public void ClearVRAM(uint value) => lastbuffer.Fill(value);
    ...
    public void ClearVRAM(int aStart, int aCount, int value) => lastbuffer.Fill(aStart, aCount, value);
    ...
    /// </summary>
    public void Swap() => IO.LinearFrameBuffer.Copy(lastbuffer);
