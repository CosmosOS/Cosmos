...
public class FileStreamImpl
{
    ...
    public static void Ctor(FileStream aThis, string aPathname, FileMode aMode,
        [FieldAccess(Name = InnerStreamFieldId)]
        ref Stream innerStream) =>
    ...
    public static long Seek(FileStream aThis,
        [FieldAccess(Name = InnerStreamFieldId)]
        ref Stream innerStream, long offset, SeekOrigin origin) =>
    ...
    {
        Global.mFileSystemDebugger.SendInternal(
            $"-- FileStream.TruncateFile -- aPath = {aPath} existing = {aPathExists}");
    ...
    {
        Global.mFileSystemDebugger.SendInternal(
            $"-- FileStream.CreateFile -- aPath = {aPath} existing = {aPathExists}");
    ...
    private static Stream AppendToFile(string aPath, bool aPathExists)
    {
        ...
        {
            Global.mFileSystemDebugger.SendInternal(
                "Append mode with aPath already existing let's seek to end of the file");
            var aStream = VFSManager.GetFileStream(aPath);
    ...
    {
        Global.mFileSystemDebugger.SendInternal(
            $"In FileStream.OpenOrCreateFile aPath {aPath} existing? {aPathExists}");

        if (aPathExists)
        {
            Global.mFileSystemDebugger.SendInternal(
                "OpenOrCreateFile Mode with aPath already existing, let's Open it!");
    ...
    private static Stream InitializeStream(string aPath, FileMode aMode)
    {
        ...
        var aPathExists = VFSManager.FileExists(aPath);
    ...
    public static bool
        get_CanWrite(FileStream aThis, [FieldAccess(Name = InnerStreamFieldId)] ref Stream innerStream) =>
    ...
    public static void WriteByte(FileStream aThis, byte aByte) => throw new NotImplementedException();
}
