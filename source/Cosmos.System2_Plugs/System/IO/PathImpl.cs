...
public static class PathImpl
{
    public static void Cctor(
        [FieldAccess(Name = "System.Char System.IO.Path.AltDirectorySeparatorChar")]
        ref char aAltDirectorySeparatorChar,
        [FieldAccess(Name = "System.Char System.IO.Path.DirectorySeparatorChar")]
        ref char aDirectorySeparatorChar,
        [FieldAccess(Name = "System.Char System.IO.Path.VolumeSeparatorChar")]
        ref char aVolumeSeparatorChar)
    {
    ...
    public static string ChangeExtension(string aPath, string aExtension)
    {
        if (aPath != null)
        {
            ...
            Global.mFileSystemDebugger.SendInternal(
                $"Path.ChangeExtension : aPath = {aPath}, aExtension = {aExtension}, returning {xText}");
            return xText;
        }
    ...
    public static string Combine(string aPath1, string aPath2)
    {
        ...
        var result = CombineNoChecks(aPath1, aPath2);
        Global.mFileSystemDebugger.SendInternal(
            $"Path.Combine : aPath1 = {aPath1}, aPath2 = {aPath2}, returning {result}");
        return result;
    }
    ...
    private static string CombineNoChecks(string aPath1, string aPath2)
    {
        ...
        if (xC != Path.DirectorySeparatorChar && xC != Path.AltDirectorySeparatorChar && xC != Path.VolumeSeparatorChar)
        {
            ...
            Global.mFileSystemDebugger.SendInternal(
                $"Path.CombineNoChecks : aPath1 = {aPath1}, aPath2 = {aPath2}, returning {xResult}");
            return xResult;
        }
    ...
    public static string GetFileNameWithoutExtension(string aPath)
    {
        ...
        var xResult = aPath.Substring(0, xLength);
    ...
    public static string GetFullPath(string aPath)
    {
        ...
        var result = NormalizePath(aPath, true);
        Global.mFileSystemDebugger.SendInternal($"Path.GetFullPath : aPath = {aPath}, returning {result}");
        return result;
    }

    public static char[] GetInvalidFileNameChars() => VFSManager.GetInvalidFileNameChars();

    public static char[] GetInvalidPathChars() => VFSManager.GetRealInvalidPathChars();
    ...
    public static string GetRandomFileName() => "random.tmp";

    public static string GetTempFileName() => "tempfile.tmp";

    public static string GetTempPath() => @"0:\Temp";
    ...
    public static bool HasIllegalCharacters(string aPath, bool aCheckAdditional)
    {
        if (aCheckAdditional)
        {
            var xInvalidWithAdditional = VFSManager.GetInvalidPathCharsWithAdditionalChecks();
            for (var i = 0; i < xInvalidWithAdditional.Length; i++)
            {
        ...
        var xInvalid = VFSManager.GetRealInvalidPathChars();
        for (var i = 0; i < xInvalid.Length; i++)
        {
            for (var j = 0; j < aPath.Length; j++)
            {
    ...
    private static bool IsDirectorySeparator(char aC)
    {
    ...
    public static string GetDirectoryName(string aPath)
    {
        ...
        if (aPath != null)
        {
            ...
            CheckInvalidPathChars(aPath);
            var xText = NormalizePath(aPath, false);
            if (aPath.Length > 0)
            {
                try
                {
            ...
            aPath = xText;
            var rootLength = GetRootLength(aPath);
            var num2 = aPath.Length;
            if (num2 > rootLength)
            {
                num2 = aPath.Length;
                if (num2 == rootLength)
                ...
                while (num2 > rootLength && aPath[--num2] != Path.DirectorySeparatorChar &&
                       aPath[num2] != Path.AltDirectorySeparatorChar)
                {
                }
    ...
    private static string NormalizePath(string aPath, bool aFullCheck)
    {
