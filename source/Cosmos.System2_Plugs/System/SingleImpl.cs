...
{
    public static string ToString(ref float aThis) => StringHelper.GetNumberString(aThis);
    ...
    public static float Parse(string s)
    {
        ...
        var internalDigits = new List<int>();
        ...
        while (s.Length != 0)
        {
            ...
            else if (active >= '0' && active <= '9')
            {
                if (foundDecimal)
                {
                    fractionalDigits.Add(Int32.Parse(active.ToString()));
                }
                else
                {
                    internalDigits.Add(Int32.Parse(active.ToString()));
                }
            }
            else
            {
        ...
        while (s.Length != 0)
        {
            ...
            if (s[0] == 'E' || s[0] == 'e')
            {
                multiplier = Single.Parse(s.Substring(1));
                break;
            }
        ...
        //Create float
        float parsed = 0;
        for (var i = 0; i < internalDigits.Count; i++)
        {
            parsed += internalDigits[i] * (float)Math.Pow(10, internalDigits.Count - (i + 1));
        }

        for (var i = 0; i < fractionalDigits.Count; i++)
        {
