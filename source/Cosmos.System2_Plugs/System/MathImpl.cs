...
public static class MathImpl
{
    ...
    internal static Debugger mDebugger = new("System", "Math Plugs");
    ...
    public static double Acos(double x)
    {
        ...
        df = s;
        //__LO(df) = 0;
        var bdf = BitConverter.GetBytes(BitConverter.DoubleToInt64Bits(df));
        for (var i = 0; i < 4; i++)
        {
    ...
    public static double Asin(double x)
    {
        ...
        else
        {
            w = s;
            //__LO(w) = 0;
            var bw = BitConverter.GetBytes(BitConverter.DoubleToInt64Bits(w));
            for (var i = 0; i < 4; i++)
            {
    ...
    public static double Cosh(double x)
    {
        ...
        return x == 0F ? 1F :
            x <= ln2 / 2 ? 1 + _power(Exp(x) - 1, 2) / (2 * Exp(x)) :
            x <= 22F ? (Exp(x) + 1 / Exp(x)) / 2 : 0.5F * (Exp(x) + Exp(-x));
    }
    ...
    public static double Exp(double x)
    {
        ...
        var hx = HighWord(x); //Highword of x

        xsb = (hx >> 31) & 1; //Get sign of x
        ...
        if (k >= -1021)
        {
            ...
            var _y = BitConverter.DoubleToInt64Bits(y);
    ...
    public static double Pow(double b, double e)
    {
        ...
        if (b < 0)
        {
            ...
            var logedBase = Log(Abs(b));
            var pow = Exp(logedBase * e);
            if ((long)e % 2 == 0)
            {
                return pow;
            }
    ...
    public static double Sqrt(double x)
    {
        ...
        var value = BitConverter.DoubleToInt64Bits(x);
        var valueBytes = BitConverter.GetBytes(value);
        var offset = BitConverter.IsLittleEndian ? 4 : 0;
    ...
    public static double Tanh(double x) => expm1(2F * x) / (expm1(2F * x) + 2F);

    #endregion Tanh

    #region Truncate

    public static double Truncate(double x) => x == 0 ? 0F : x > 0F ? Floor(x) : Ceiling(x);
    ...
    {
        var highOffset = BitConverter.IsLittleEndian ? 4 : 0;
        var lowOffset = BitConverter.IsLittleEndian ? 4 : 0;
        ...
        var lv = BitConverter.DoubleToInt64Bits(ix - (e0 << 20));
        ...
        */
        ...
        else
        {
            ...
            value = ((((atan_p4 * argsq + atan_p3) * argsq + atan_p2) * argsq + atan_p1) * argsq + atan_p0) /
                    (((((argsq + atan_q4) * argsq + atan_q3) * argsq + atan_q2) * argsq + atan_q1) * argsq + atan_q0);
        }
