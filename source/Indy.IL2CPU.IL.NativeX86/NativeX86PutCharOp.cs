...
namespace Indy.IL2CPU.IL.NativeX86 {
	public class NativeX86PutCharOp: IL.X86.Op {
		...
		public override void DoAssemble() {
			...
			AssemblyDefinition xAsm = AssemblyFactory.GetAssembly(typeof(NativeX86PutCharOp).Assembly.Location);
			foreach (ModuleDefinition xMod in xAsm.Modules) {
				if (xMod.Types.Contains(typeof(NativeX86PutCharOp).FullName)) {
					xType = xMod.Types[typeof(NativeX86PutCharOp).FullName];
					break;
				}
			}
			if (xType == null) {
