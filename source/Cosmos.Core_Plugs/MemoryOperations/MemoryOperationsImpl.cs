...
[Plug(Target = typeof(Core.MemoryOperations))]
public static unsafe class MemoryOperationsImpl
{
    [PlugMethod(Assembler = typeof(MemoryOperationsFill16BlocksAsm))]
    private static void Fill16Blocks(byte* dest, int value, int BlocksNum) => throw new NotImplementedException();
    ...
    private static void Copy16Bytes(byte* dest, byte* src) => throw new NotImplementedException();

    [PlugMethod(Assembler = typeof(MemoryOperationsCopy32BytesAsm))]
    private static void Copy32Bytes(byte* dest, byte* src) => throw new NotImplementedException();

    [PlugMethod(Assembler = typeof(MemoryOperationsCopy64BytesAsm))]
    private static void Copy64Bytes(byte* dest, byte* src) => throw new NotImplementedException();

    [PlugMethod(Assembler = typeof(MemoryOperationsCopy128BytesAsm))]
    private static void Copy128Bytes(byte* dest, byte* src) => throw new NotImplementedException();

    [PlugMethod(Assembler = typeof(MemoryOperationsCopy128BlocksAsm))]
    private static void Copy128Blocks(byte* dest, byte* src, int blockNums) => throw new NotImplementedException();

    /*
     * Tiny memory copy with jump table optimized
     */
    private static void CopyTiny(byte* dest, byte* src, int size)
    {
...
#endif
        Global.mDebugger.SendInternal(
            $"size {size} is composed of {xBlocksNum} blocks of {xBlockSize} bytes with {xByteRemaining} remainder");

        // TODO call Copy128Blocks()
        for (var i = 0; i < xByteRemaining; i++)
        {
            *(dest + i) = *(src + i);
        }
