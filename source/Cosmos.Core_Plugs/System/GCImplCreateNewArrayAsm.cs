...
internal class GCImplCreateNewArrayAsm : AssemblerMethod
{
    // this code is mostly copied from Newarr.cs in Il2CPU, just the code to find the size and length is different
    public override void AssembleNew(Assembler aAssembler, object aMethodInfo)
    {
        var xTypeID = ILOp.GetTypeIDLabel(typeof(Array));
        MethodBase xCtor = typeof(Array).GetConstructors(BindingFlags.NonPublic | BindingFlags.Public |
                                                         BindingFlags.Static | BindingFlags.Instance)[0];
        var xCtorName = LabelName.Get(xCtor);
        ...
        XS.Set(EAX, EBX, true); // array type id
