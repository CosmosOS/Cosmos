...
public static class ArrayImpl
{
    ...
    {
        var xElementSize = aArray[3];
        aArray += 4;
        var xBytes = (byte*)aArray;
        for (var i = aIndex * xElementSize; i < (aIndex + aLength) * xElementSize; i++)
        {
            xBytes[i] = 0;
        }
    }

    public static int GetUpperBound(Array aThis, int aDimension) => GetLength(aThis, aDimension) - 1;
    ...
    [PlugMethod(Signature =
        "System_Boolean__System_Array_TrySZBinarySearch_System_Array__System_Int32__System_Int32__System_Object___System_Int32_")]
    public static unsafe bool TrySZBinarySearch([ObjectPointerAccess] uint* aArray, uint sourceIndex, uint count,
        uint value, out uint retVal)
    {
    ...
    [PlugMethod(Signature =
        "System_Boolean__System_Array_TrySZLastIndexOf_System_Array__System_Int32__System_Int32__System_Object___System_Int32_")]
    public static unsafe bool TrySZLastIndexOf([ObjectPointerAccess] uint* aArray, uint sourceIndex, uint count,
        uint value, out uint retVal)
    {
        aArray = (uint*)aArray[0];
        aArray += 4;
        for (var i = sourceIndex + count; i > sourceIndex; i--)
        {
            if (aArray[i - 1] == value)
            {
                retVal = i - 1;
    ...
    private static unsafe bool TrySZIndexOf([ObjectPointerAccess] uint* aArray, uint sourceIndex, uint count,
        uint value, out uint retVal)
    {
        aArray = (uint*)aArray[0];
        aArray += 4;
    ...
    public static unsafe int get_Rank([ObjectPointerAccess] int* aThis) => 1;
    ...
    {
        var xElementSize = aThis[3];
        aThis += 4;
        var aArray = (byte*)aThis;
    ...
    public static object InternalGetValue(Array aThis, IntPtr aIntPtr) => aThis.GetValue((int)aIntPtr);
}
