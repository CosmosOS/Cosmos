...
public class DelegateInvokeAsm : AssemblerMethod
{
    public override void AssembleNew(Assembler aAssembler, object aMethodInfo)
    {
        ...
        */
        ...
        var xArgSizeOffset = Ldfld.GetFieldOffset(typeof(Delegate), "$$ArgSize$$");
        Ldarg.DoExecute(aAssembler, xMethodInfo, 0);
        XS.Add(ESP, 4);
        XS.Pop(ECX);
        ...
        var returnSizeOffset = Ldfld.GetFieldOffset(typeof(Delegate), "$$ReturnSize$$");
        Ldarg.DoExecute(aAssembler, xMethodInfo, 0);
        XS.Add(ESP, 4);
        XS.Pop(ESI);
        ...
        {
            XS.Compare(EDX, EBX);
            XS.Jump(ConditionalTestEnum.GreaterThanOrEqualTo, ".END_OF_INVOKE");
            XS.PushAllRegisters();
            ...
            XS.Add(EDI, 4);
            XS.Set(EDI, EDI,
                sourceDisplacement: Ldfld.GetFieldOffset(xMethodInfo.MethodBase.DeclaringType,
                    "System.Object System.Delegate._target"));
            ...
            XS.Compare(EDI, 1);
            XS.Jump(ConditionalTestEnum.Equal, ".Struct"); //structs are just the pointer so we are already done
            XS.Push(0);
            XS.Add(EDX, 4);
            XS.Jump(".NO_THIS");
            XS.Label(".Struct");
            XS.Add(ESP, ObjectUtils.FieldDataOffset, true);
            XS.Label(".NO_THIS");
            XS.Set(EDI, EAX, sourceIsIndirect: true, sourceDisplacement: 4);
            XS.Set(EDI, EDI,
                sourceDisplacement: Ldfld.GetFieldOffset(xMethodInfo.MethodBase.DeclaringType,
                    "System.IntPtr System.Delegate._methodPtr"));

            XS.Set(EBX, 0); // initialise required extra space to 0
            XS.Compare(ESI, EDX);
            XS.Jump(ConditionalTestEnum.LessThanOrEqualTo, ".NO_RETURN_VALUE_SPACE");
            XS.Set(EBX, ESI);
            XS.Sub(EBX, ECX);
            ...
            XS.Compare(ECX, 0);
            XS.Jump(ConditionalTestEnum.Zero, ".NO_ARGS");
            XS.Label(".HAS_ARGS");
            XS.Sub(ESP, ECX);
            XS.Push(EDI);
            XS.Set(EDI, ESP);
            ...
            XS.Set(ESI, EBP);
            XS.Compare(EBX, 0);
            XS.Jump(ConditionalTestEnum.Equal, ".NO_RETURN_EXTRA");
            ...
            XS.Compare(EAX, 0);
            XS.Jump(ConditionalTestEnum.LessThanOrEqualTo, ".RETURN_VALUE_LOOP_END");
            XS.Pop(EBX);
            XS.Set(EDI, EBX, true);
            XS.Add(EDI, 4);
            XS.Sub(EAX, 4);
