...
{
    private static readonly Debugger mDebugger = new("Core", "Memory");

    private static uint mEndOfRam;

    private static uint mLastTableIndex;

    private static uint mLastEntryIndex;
    ...
    public static uint MemAlloc(uint aLength)
    {
        ...
        try
        {
            ...
            var xCurrentTable = GlobalSystemInfo.GlobalInformationTable->FirstDataLookupTable;
            ...
            if (!ScanDataLookupTable(xCurrentTableIdx, xNextTablePointer, aLength, out xResult))
            {
                ...
                mDebugger.Send(
                    "    Something seriously weird happened: we could create a new DataLookupTable (with new entries), but couldn't allocate a new handle from it.");
                while (true)
                {
                }
            }
    ...
    private static bool ScanDataLookupTable(uint aTableIdx, DataLookupTable* aTable, uint aSize, out uint aHandle)
    {
        ...
        for (var i = mLastEntryIndex; i < DataLookupTable.EntriesPerTable; i++)
        {
