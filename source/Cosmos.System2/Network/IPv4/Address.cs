...
public class Address : IComparable
{
    ...
    /// </summary>
    public static Address Zero = new(0, 0, 0, 0);

    /// <summary>
    ///     Broadcast address (255.255.255.255).
    /// </summary>
    public static Address Broadcast = new(255, 255, 255, 255);
    ...
    ///     Thrown if adr represents a number less than Byte.MinValue or greater than
    ///     Byte.MaxValue.
    /// </exception>
    public static Address Parse(string adr)
    {
        var fragments = adr.Split('.');
        if (fragments.Length == 4)
        {
            try
            {
                var first = Byte.Parse(fragments[0]);
                var second = Byte.Parse(fragments[1]);
                var third = Byte.Parse(fragments[2]);
                var fourth = Byte.Parse(fragments[3]);
    ...
    public static Address CIDRToAddress(int cidr)
    {
        try
        {
            ...
            return new Address((byte)(mask >> 24), (byte)((mask >> 16) & 0xff), (byte)((mask >> 8) & 0xff),
                (byte)(mask & 0xff));
        }
        catch
        {
    ...
    /// </summary>
    /// <returns></returns>
    public byte[] ToByteArray() => address;
    ...
    private uint to32BitNumber() =>
        (uint)((address[0] << 24) | (address[1] << 16) | (address[2] << 8) | (address[3] << 0));
}
