...
public class IPPacket : EthernetPacket
{
    ...
    internal static void IPv4Handler(byte[] packetData)
    {
        ...
        if (NetworkStack.AddressMap.ContainsKey(ip_packet.DestinationIP.Hash) ||
    ...
    protected static ushort CalcOcCRC(byte[] buffer, ushort offset, int length) =>
        (ushort)~SumShortValues(buffer, offset, length);
    ...
    protected static ushort SumShortValues(byte[] buffer, int offset, int length)
    {
        ...
        while (i != end)
        {
            chksum += (uint)((buffer[i++] << 8) + buffer[i++]);
        }

        if (i != offset + length)
        {
            chksum += (uint)(buffer[i] << 8);
        }
    ...
    /// <returns>ushort value.</returns>
    protected ushort CalcIPCRC(ushort headerLength) => CalcOcCRC(14, headerLength);

    /// <summary>
    ///     To string.
    /// </summary>
    /// <returns>string value.</returns>
    public override string ToString() => "IP Packet Src=" + SourceIP + ", Dest=" + DestinationIP + ", Protocol=" +
                                         Protocol + ", TTL=" + TTL + ", DataLen=" + DataLength;
}
