...
public class ICMPClient : IDisposable
{
    ...
    ///     Clients dictionary.
    /// </summary>
    private static readonly Dictionary<uint, ICMPClient> clients;
    ...
    public void Close()
    {
        if (clients.ContainsKey(destination.Hash))
        {
    ...
    {
        var source = IPConfig.FindNetwork(destination);
    ...
    public void ReceiveData(ICMPPacket packet) => rxBuffer.Enqueue(packet);
}
