...
internal static class OutgoingBuffer
{
    ...
    {
        EnsureQueueExists();
        var nic = IPConfig.FindInterface(packet.SourceIP);
            ...
            for (var e = 0; e < queue.Count; e++)
            {
                ...
                if (entry.Status == BufferEntry.EntryStatus.ADDED)
                {
                    if (IPConfig.IsLocalAddress(entry.Packet.DestinationIP) == false)
                    {
                        ...
                        {
                            var arp_request = new ARPRequest_Ethernet(entry.NIC.MACAddress, entry.Packet.SourceIP,
    ...
    private class BufferEntry
    {
        ...
        /// </summary>
        public readonly NetworkDevice NIC;

        /// <summary>
        ///     IP packet.
        /// </summary>
        public readonly IPPacket Packet;
