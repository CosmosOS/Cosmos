...
public class TCPPacket : IPPacket
{
    ...
    internal byte[] TCP_Data
    {
        get
        {
            var data = new byte[TCP_DataLength];
    ...
    internal static void TCPHandler(byte[] packetData)
    {
        ...
        if (packet.CheckCRC())
        {
            var connection = Tcp.GetConnection(packet.DestinationPort, packet.SourcePort, packet.DestinationIP,
                packet.SourceIP);
        ...
        ushort WSValue, ushort UrgentPointer)
    {
        ...
        var calculatedcrc = CalcOcCRC(header, 0, header.Length);
    ...
    protected override void InitFields()
    {
        ...
        SequenceNumber = (uint)((RawData[DataOffset + 4] << 24) | (RawData[DataOffset + 5] << 16) |
                                (RawData[DataOffset + 6] << 8) | RawData[DataOffset + 7]);
        AckNumber = (uint)((RawData[DataOffset + 8] << 24) | (RawData[DataOffset + 9] << 16) |
                           (RawData[DataOffset + 10] << 8) | RawData[DataOffset + 11]);
        ...
        if (TCPHeaderLength > 20) //options
        {
            ...
            for (var i = 0; i < TCP_DataLength; i++)
            {
                ...
                if (option.Kind != 1) //NOP
                {
                    ...
                    if (option.Length != 2)
                    {
                        ...
                        for (var j = 0; j < option.Length - 2; j++)
                        {
    ...
    {
        var header = new byte[12 + TCPHeaderLength + TCP_DataLength];
        ...
        //Protocol (TCP)
        header[9] = 0x06;
        var tcplen = (ushort)(TCPHeaderLength + TCP_DataLength);
        //TCP Length
