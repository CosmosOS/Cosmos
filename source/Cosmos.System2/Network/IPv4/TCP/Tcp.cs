...
public enum Status
{
    ...
    /// <summary>
    ///     Represents an open connection, data received can be delivered to the user. The normal state for the data transfer
    ///     phase of the connection.
    /// </summary>
    ESTABLISHED,

    /// <summary>
    ///     Wait for a connection termination request from the remote TCP, or an acknowledgment of the connection termination
    ...
    /// <summary>
    ///     Wait for an acknowledgment of the connection termination request previously sent to the remote TCP (which includes
    ///     an acknowledgment of its connection termination request).
    /// </summary>
    LAST_ACK,

    /// <summary>
    ///     Wait for enough time to pass to be sure the remote TCP received the acknowledgment of its connection termination
    ///     request.
    /// </summary>
    TIME_WAIT,
...
///     Handle received packets according to current TCP connection Status. Also contains TCB (Transmission Control Block)
///     information.
...
public class Tcp
{
    ...
    {
        Global.mDebugger.Send("[" + table[(int)Status] + "] " + packet);
        ...
        else
        {
            ...
            if (TCB.RcvNxt <= packet.SequenceNumber &&
                packet.SequenceNumber + packet.TCP_DataLength < TCB.RcvNxt + TCB.RcvWnd)
            {
                switch (Status)
                {
    ...
    public void ProcessEstablished(TCPPacket packet)
    {
        if (packet.ACK)
        {
            if (TCB.SndUna < packet.AckNumber && packet.AckNumber <= TCB.SndNxt)
            {
                ...
                //Update Window Size
                if (TCB.SndWl1 < packet.SequenceNumber ||
                    (TCB.SndWl1 == packet.SequenceNumber && TCB.SndWl2 <= packet.AckNumber))
                {
                    TCB.SndWnd = packet.WindowSize;
                    TCB.SndWl1 = packet.SequenceNumber;
                    TCB.SndWl2 = packet.AckNumber;
    ...
    ///     Send empty packet.
    /// </summary>
    internal void SendEmptyPacket(Flags flag) => SendPacket(new TCPPacket(LocalEndPoint.Address, RemoteEndPoint.Address,
        LocalEndPoint.Port, RemoteEndPoint.Port, TCB.SndNxt, TCB.RcvNxt, 20, (byte)flag, TCB.SndWnd, 0));

    /// <summary>
    ///     Send empty packet.
    /// </summary>
    internal void SendEmptyPacket(Flags flag, uint sequenceNumber) => SendPacket(new TCPPacket(LocalEndPoint.Address,
        RemoteEndPoint.Address, LocalEndPoint.Port, RemoteEndPoint.Port, sequenceNumber, TCB.RcvNxt, 20, (byte)flag,
        TCB.SndWnd, 0));
