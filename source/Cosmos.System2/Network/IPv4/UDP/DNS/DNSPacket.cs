...
public class DNSPacket : UDPPacket
{
    ...
    {
        var rnd = new Random();
        var transactionID = (byte)rnd.Next(0, Int32.MaxValue);
        RawData[DataOffset + 8] = (byte)((transactionID >> 8) & 0xFF);
        RawData[DataOffset + 9] = (byte)((transactionID >> 0) & 0xFF);

        RawData[DataOffset + 10] = (0x0100 >> 8) & 0xFF;
        RawData[DataOffset + 11] = (0x0100 >> 0) & 0xFF;

        RawData[DataOffset + 12] = (byte)((urlnb >> 8) & 0xFF);
        RawData[DataOffset + 13] = (byte)((urlnb >> 0) & 0xFF);

        RawData[DataOffset + 14] = (0 >> 8) & 0xFF;
        RawData[DataOffset + 15] = (0 >> 0) & 0xFF;

        RawData[DataOffset + 16] = (0 >> 8) & 0xFF;
        RawData[DataOffset + 17] = (0 >> 0) & 0xFF;

        RawData[DataOffset + 18] = (0 >> 8) & 0xFF;
        RawData[DataOffset + 19] = (0 >> 0) & 0xFF;
    ...
    {
        var dns_packet = new DNSPacket(packetData);

        var receiver = (DnsClient)UdpClient.GetClient(dns_packet.DestinationPort);
        if (receiver != null)
        {
    ...
    public override string ToString() => "DNS Packet Src=" + SourceIP + ":" + SourcePort + ", Dest=" + DestinationIP +
                                         ":" + DestinationPort;
}
...
public class DNSPacketAsk : DNSPacket
{
        ...
        : base(source, dest, 1, (ushort)(5 + url.Length + 1))
    {
        ...
        {
            var word = Encoding.ASCII.GetBytes(item);

            RawData[DataOffset + 20 + b] = (byte)word.Length; //set word length

            b++;

            foreach (var letter in word)
            {
                RawData[DataOffset + 20 + b] = letter;
                b++;
            }
        }

        RawData[DataOffset + 20 + b] = 0x00;

        RawData[DataOffset + 20 + b + 1] = 0x00;
        RawData[DataOffset + 20 + b + 2] = 0x01;

        RawData[DataOffset + 20 + b + 3] = 0x00;
        RawData[DataOffset + 20 + b + 4] = 0x01;
    }
}
    ...
    protected override void InitFields()
    {
        ...
        if (AnswerRRs > 0)
        {
            ...
            for (var i = 0; i < AnswerRRs; i++)
            {
                ...
                answer.TimeToLive = (RawData[index + 6] << 24) | (RawData[index + 7] << 16) |
                                    (RawData[index + 8] << 8) | RawData[index + 9];
                answer.DataLenght = (ushort)((RawData[index + 10] << 8) | RawData[index + 11]);
                index += 12;
                answer.Address = new byte[answer.DataLenght];
                for (var j = 0; j < answer.DataLenght; j++, index++)
                {
