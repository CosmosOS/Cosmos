...
public class DHCPClient : UdpClient
{
    ...
    public static Address DHCPServerAddress(NetworkDevice networkDevice) =>
        NetworkConfiguration.Get(networkDevice).DefaultGateway;
    ...
    private int Receive(int timeout = 5000)
    {
        ...
        if (packet.MessageType == 2) //Boot Reply
        {
            ...
            if (packet.RawData[284] == 0x05 || packet.RawData[284] == 0x06) //ACK or NAK DHCP packet received
            {
    ...
    {
        foreach (var networkDevice in NetworkDevice.Devices)
        {
            var source = IPConfig.FindNetwork(DHCPServerAddress(networkDevice));
    ...
    /// <param name="message">
    ///     Enable/Disable the displaying of messages about DHCP applying and conf. Disabled by default.
    /// </param>
    private void Apply(DHCPAck packet, bool message = false)
    {
        ...
        foreach (var networkDevice in NetworkDevice.Devices)
        {
            ...
            if (message)
            {
                Global.mDebugger.Send("[DHCP ACK][" + networkDevice.Name +
                                      "] Packet received, applying IP configuration...");
                Global.mDebugger.Send("   IP Address  : " + packet.Client);
                Global.mDebugger.Send("   Subnet mask : " + packet.Subnet);
                Global.mDebugger.Send("   Gateway     : " + packet.Server);
                Global.mDebugger.Send("   DNS server  : " + packet.DNS);
            }
