...
public class ICMPPacket : IPPacket
{
    ...
    internal static void ICMPHandler(byte[] packetData)
    {
        ...
        switch (icmp_packet.ICMPType)
        {
            case 0:
                ...
                Global.mDebugger.Send("Received ICMP Echo reply from " + icmp_packet.SourceIP);
                break;
            case 8:
                var request = new ICMPEchoRequest(packetData);
                var reply = new ICMPEchoReply(request);
                Global.mDebugger.Send("Sending ICMP Echo reply to " + reply.DestinationIP);
    ...
    /// <returns></returns>
    protected ushort CalcICMPCRC(ushort length) => CalcOcCRC(DataOffset, length);
    ...
    {
        var data = new byte[ICMPDataLength];

        for (var b = 0; b < ICMPDataLength; b++)
        {
    ...
    public override string ToString() => "ICMP Packet Src=" + SourceIP + ", Dest=" + DestinationIP + ", Type=" +
                                         icmpType + ", Code=" + icmpCode;
}
...
internal class ICMPEchoRequest : ICMPPacket
{
    ...
    {
        for (var b = 8; b < ICMPDataLength; b++)
        {
    ...
    /// </summary>
    /// <returns>string value.</returns>
    public override string ToString() => "ICMP Echo Request Src=" + SourceIP + ", Dest=" + DestinationIP + ", ID=" +
                                         icmpID + ", Sequence=" + icmpSequence;
}
...
internal class ICMPEchoReply : ICMPPacket
{
    ...
    internal ICMPEchoReply(ICMPEchoRequest request)
        : base(request.DestinationIP, request.SourceIP, 0, 0, request.ICMPID, request.ICMPSequence,
            request.ICMPDataLength)
    {
        for (var b = 0; b < ICMPDataLength; b++)
        {
    ...
    /// </summary>
    /// <returns>string value.</returns>
    public override string ToString() => "ICMP Echo Reply Src=" + SourceIP + ", Dest=" + DestinationIP + ", ID=" +
                                         icmpID + ", Sequence=" + icmpSequence;
}
