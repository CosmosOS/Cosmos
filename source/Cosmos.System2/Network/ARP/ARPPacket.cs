...
public class ARPPacket : EthernetPacket
{
    ...
    {
        var arp_packet = new ARPPacket(packetData);
        ...
        if (arp_packet.Operation == 0x01)
        {
            ...
            {
                var arp_request = new ARPRequest_Ethernet(packetData);
                ...
                if (NetworkStack.AddressMap.ContainsKey(arp_request.TargetIP.Hash))
                {
                    Global.mDebugger.Send("ARP Request Recvd from " + arp_request.SenderIP);
                    var nic = NetworkStack.AddressMap[arp_request.TargetIP.Hash];

                    var reply =
                        new ARPReply_Ethernet(nic.MACAddress, arp_request.TargetIP, arp_request.SenderMAC,
                            arp_request.SenderIP);
        ...
        else if (arp_packet.Operation == 0x02)
        {
            if (arp_packet.HardwareType == 1 && arp_packet.ProtocolType == 0x0800)
            {
                var arp_reply = new ARPReply_Ethernet(packetData);
                Global.mDebugger.Send("Received ARP Reply");
                Global.mDebugger.Send(arp_reply.ToString());
                Global.mDebugger.Send("ARP Reply Recvd from " + arp_reply.SenderIP);
    ...
    public override string ToString() =>
        "ARP Packet Src=" + srcMAC + ", Dest=" + destMAC + ", HWType=" + aHardwareType + ", Protocol=" + aProtocolType +
        ", Operation=" + Operation;
}
