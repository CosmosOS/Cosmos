...
public class CosmosVFS : VFSBase
{
    ...
    public override DirectoryEntry CreateFile(string aPath)
    {
        ...
        var xFileToCreate = Path.GetFileName(aPath);
        ...
        var xParentDirectory = Path.GetDirectoryName(aPath);
    ...
    public override DirectoryEntry CreateDirectory(string aPath)
    {
        ...
        var xDirectoryToCreate = Path.GetFileName(aPath);
    ...
    public override DirectoryEntry GetDirectory(string aPath)
    {
        ...
        catch (Exception)
        {
            Global.mFileSystemDebugger.SendInternal(
                "CosmosVFS.GetDirectory - DoGetDirectoryEntry failed, returning null. aPath = " + aPath);
            return null;
        }
    ...
    public override DirectoryEntry GetFile(string aPath)
    {
        ...
        {
            Global.mFileSystemDebugger.SendInternal(
                "CosmosVFS.GetFile - DoGetDirectoryEntry failed, returning null. aPath = " + aPath);
            return null;
        }
    ...
    private FileSystem GetFileSystemFromPath(string aPath)
    {
        ...
        var xPath = Path.GetPathRoot(aPath);
    ...
    private DirectoryEntry DoGetDirectoryEntry(string aPath, FileSystem aFS)
    {
        ...
        var xPathParts = VFSManager.SplitPath(aPath);

        var xBaseDirectory = GetVolume(aFS);
        ...
        for (var i = 1; i < xPathParts.Length; i++)
        {
            ...
            for (var j = 0; j < xListing.Count; j++)
            {
                var xListingItem = xListing[j];
                var xListingItemName = xListingItem.mName.ToLower();
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    ///     Thrown if driveId length is smaller then 2, or greater than
    ///     Int32.MaxValue.
    /// </exception>
    public override bool IsValidDriveId(string driveId)
    {
        ...
        */
        ...
        var isOK = Int32.TryParse(driveId, out var val);
