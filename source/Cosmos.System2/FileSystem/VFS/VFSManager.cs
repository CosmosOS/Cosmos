...
public static class VFSManager
{
    ...
    public static DirectoryEntry GetFile(string aPath)
    {
        ...
        var xFileName = Path.GetFileName(aPath);
        Global.mFileSystemDebugger.SendInternal("xFileName = " + xFileName);

        var xDirectory = aPath.Remove(aPath.Length - xFileName.Length);
    ...
    ///     Thrown if specified path isn't a directory / trying to delete not empty directory not
    ///     recursivly / directory contains a corrupted file.
    /// </exception>
    ...
    public static bool FileExists(string aPath)
    {
        ...
        Global.mFileSystemDebugger.SendInternal("aPath = " + aPath);
        var xPath = Path.GetFullPath(aPath);
    ...
    public static DirectoryEntry GetParent(string aPath)
    {
        ...
        var xFileOrDirectory = Path.HasExtension(aPath) ? Path.GetFileName(aPath) : Path.GetDirectoryName(aPath);
        if (xFileOrDirectory != null)
        {
            var xPath = aPath.Remove(aPath.Length - xFileOrDirectory.Length);
    ...
    /// </summary>
    internal static void Reset() => mVFS = null;
    ...
    public static void ThrowIfNotRegistered()
    {
        ...
        {
            throw new Exception(
                "VFS not registered. Use VFSManager.RegisterVFS() before doing file system operations!");
        }
    }
    ...
    public static int GetMaxPath() => 260;
    ...
    public static string[] SplitPath(string aPath) =>
        ...
        aPath.Split(GetDirectorySeparators(), StringSplitOptions.RemoveEmptyEntries);
    ...
    private static char[] GetDirectorySeparators() =>
        new[] { GetDirectorySeparatorChar(), GetAltDirectorySeparatorChar() };
