...
internal class FatStream : Stream
{
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    ///     Thrown if aCount or aOffset is smaller than 0 or bigger than
    ///     Int32.MaxValue.
    /// </exception>
    ...
    public override int Read(byte[] aBuffer, int aOffset, int aCount)
    {
        ...
        for (var i = 0; i < mFatTable.Length; i++)
        {
        ...
        var xMaxReadableBytes = mDirectoryEntry.mSize - mPosition;
        long xCount = aCount;
        ...
        while (xCount > 0)
        {
            var xClusterIdx = mPosition / xClusterSize;
            var xPosInCluster = mPosition % xClusterSize;
            mFS.Read(mFatTable[(int)xClusterIdx], out var xCluster);
            long xReadSize;
    ...
    public override void Write(byte[] aBuffer, int aOffset, int aCount)
    {
        ...
        var xTotalLength = mPosition + xCount;
        ...
        while (xCount > 0)
        {
            ...
            mFS.Read(mFatTable[xClusterIdx], out var xCluster);
