...
internal class FatDirectoryEntry : DirectoryEntry
{
    ...
    public FatDirectoryEntry(FatFileSystem aFileSystem, FatDirectoryEntry aParent, string aFullPath, string aName,
        long aSize,
        uint aFirstCluster, uint aEntryHeaderDataOffset, DirectoryEntryTypeEnum aEntryType, bool aNew = false) : base(
        aFileSystem, aParent, aFullPath, aName, aSize, aEntryType)
    {
        if (aFirstCluster < aFileSystem.RootCluster)
        {
            Global.mFileSystemDebugger.SendInternal(
                $"aFirstCluster {aFirstCluster} < aFileSystem.RootCluster {aFileSystem.RootCluster}");
    ...
    /// <exception cref="ArgumentException">Thrown when aFullPath or aName is null or empty.</exception>
    public FatDirectoryEntry(FatFileSystem aFileSystem, FatDirectoryEntry aParent, string aFullPath, string aName,
        long aSize, uint aFirstCluster, bool aNew = false)
    ...
    private void InitialiseNewDirectory(FatFileSystem aFileSystem)
    {
        ...
        var dotdot = new FatDirectoryEntry(aFileSystem, this, mFullPath + "\\..", "..", 0,
            ((FatDirectoryEntry)mParent).mFirstClusterNum, 32, DirectoryEntryTypeEnum.Directory);
    ...
    private void AllocateDirectoryEntry(string aShortName, bool special)
    {
        ...
        {
            SetDirectoryEntryMetadataValue(FatDirectoryEntryMetadata.Attributes,
    ...
    public FatDirectoryEntry AddDirectoryEntry(string aName, DirectoryEntryTypeEnum aEntryType)
    {
        ...
        {
            var xShortName = aName;
            uint[] xDirectoryEntriesToAllocate = null;

            #region Long Name

            if ((aEntryType == DirectoryEntryTypeEnum.File && aName.Contains(".") &&
                 (aName.Substring(0, aName.LastIndexOf('.')).Contains(".") ||
                  aName.Substring(0, aName.IndexOf('.')).Length > 8 ||
                  aName.Substring(aName.IndexOf('.') + 1).Length > 3)) ||
                (aEntryType == DirectoryEntryTypeEnum.Directory && aName.Length > 11))
            {
                var xLongName = aName;

                var xLastPeriodPosition = aName.LastIndexOf('.');
                ...
                for (var i = xShortName.Length - 1; i > 0; i--)
                {
                    var xChar = xShortName[i];
                ...
                char[] xInvalidShortNameChars =
                    { '"', '*', '+', ',', '.', '/', ':', ';', '<', '=', '>', '?', '[', '\\', ']', '|' };

                //Remove all invalid characters
                foreach (var xInvalidChar in xInvalidShortNameChars)
                {
                    xShortName.Replace(xInvalidChar, '_');
                }

                var n = 1;
                var xDirectoryEntries = ReadDirectoryContents(true);
                var xShortFilenames = new string[xDirectoryEntries.Count];

                for (var i = 0; i < xDirectoryEntries.Count; i++)
                {
                ...
                do
                {
                    ...
                    foreach (var name in xShortFilenames)
                    {
                        if (name == xNameTry)
                        {
                            xTest = true;
                ...
                var xChecksum = CalculateChecksum(GetShortName(xShortName));

                var xNumEntries = (int)Math.Ceiling(xLongName.Length / 13d);

                var xLongNameWithPad = new char[xNumEntries * 13];
                ...
                for (var i = xNumEntries - 1; i >= 0; i--)
                {
                    var xEntry = xDirectoryEntriesToAllocate[xNumEntries - i - 1];

                    SetLongFilenameEntryMetadataValue(xEntry,
                        FatDirectoryEntryMetadata.LongFilenameEntryMetadata.SequenceNumberAndAllocationStatus,
                        (i + 1) | (i == xNumEntries - 1 ? 1 << 6 : 0));
                    SetLongFilenameEntryMetadataValue(xEntry,
                        FatDirectoryEntryMetadata.LongFilenameEntryMetadata.Attributes,
                        FatDirectoryEntryAttributeConsts.LongName);
                    SetLongFilenameEntryMetadataValue(xEntry,
                        FatDirectoryEntryMetadata.LongFilenameEntryMetadata.Checksum, xChecksum);
                    ...
                    SetLongFilenameEntryMetadataValue(xEntry,
                        FatDirectoryEntryMetadata.LongFilenameEntryMetadata.LongName1, a1);
                    SetLongFilenameEntryMetadataValue(xEntry,
                        FatDirectoryEntryMetadata.LongFilenameEntryMetadata.LongName2, a2);
                    SetLongFilenameEntryMetadataValue(xEntry,
                        FatDirectoryEntryMetadata.LongFilenameEntryMetadata.LongName3, a3);
                }
            }

            #endregion Long Name

            var xFullPath = Path.Combine(mFullPath, aName);
            var xFirstCluster = ((FatFileSystem)mFileSystem).GetFat(0).GetNextUnallocatedFatEntry();
            var xEntryHeaderDataOffset = xDirectoryEntriesToAllocate == null
                ? GetNextUnallocatedDirectoryEntry()
                : xDirectoryEntriesToAllocate[xDirectoryEntriesToAllocate.Length - 1];
            ...
            ((FatFileSystem)mFileSystem).Write(xFirstCluster,
                new byte[((FatFileSystem)mFileSystem)
                    .BytesPerCluster]); // clear the cluster where directory info will be stored

            var xNewEntry = new FatDirectoryEntry((FatFileSystem)mFileSystem, this, xFullPath, aName, 0, xFirstCluster,
                xEntryHeaderDataOffset, aEntryType, true);
    ...
    public void DeleteDirectoryEntry()
    {
        ...
        SetDirectoryEntryMetadataValue(FatDirectoryEntryMetadata.FirstByte,
            FatDirectoryEntryAttributeConsts.UnusedOrDeletedEntry);
    ...
    /// <summary>
    ///     Retrieves a <see cref="List{T}" /> of <see cref="FatDirectoryEntry" /> objects that represent the Directory Entries
    ...
    public List<FatDirectoryEntry> ReadDirectoryContents(bool aReturnShortFilenames = false)
    {
        ...
        for (uint i = 0; i < xData.Length; i += 32)
        {
            ...
            if (xAttrib == FatDirectoryEntryAttributeConsts.LongName)
            {
                ...
                {
                    Global.mFileSystemDebugger.SendInternal("<DELETED> : Attrib = " + xAttrib + ", Status = " +
                                                            xStatus);
                    continue;
                }
                ...
                if (xType == 0)
                {
                    ...
                    var xLongPart = Encoding.Unicode.GetString(xData, (int)i + 1, 10);
            ...
            else
            {
                ...
                switch (xStatus)
                {
                    ...
                    case FatDirectoryEntryAttributeConsts.UnusedOrDeletedEntry:
                        ...
                        continue;
                    default:
                        var xTest = xAttrib & (FatDirectoryEntryAttributeConsts.Directory |
                                               FatDirectoryEntryAttributeConsts.VolumeID);
                        ...
                        if (xStatus >= 0x20)
                        {
                            if (xLongName.Length > 0)
                            {
                                ...
                                xName = xLongName.Trim('\0', '\uffff').Trim();

                                //If there are trailing periods
                                var nameIndex = xName.Length - 1;
                            ...
                            else
                            {
                                if (xTest == 0)
                                {
                                    var xEntry = Encoding.ASCII.GetString(xData, (int)i, 11);
                                    xName = xEntry.Substring(0, 8).TrimEnd();
                                    var xExt = xEntry.Substring(8, 3).TrimEnd();
                        ...
                        var xFirstCluster = (uint)((BitConverter.ToUInt16(xData, (int)i + 20) << 16) |
                                                   BitConverter.ToUInt16(xData, (int)i + 26));
                        if (xTest == 0)
                        {
                            var xSize = BitConverter.ToUInt32(xData, (int)i + 28);
                            ...
                            var xFullPath = Path.Combine(mFullPath, xName);
                            var xEntry = new FatDirectoryEntry((FatFileSystem)mFileSystem, xParent, xFullPath, xName,
                                xSize, xFirstCluster, i, DirectoryEntryTypeEnum.File);
                            xResult.Add(xEntry);
                        ...
                        else if (xTest == FatDirectoryEntryAttributeConsts.Directory)
                        {
                            ...
                            var xSize = BitConverter.ToUInt32(xData, (int)i + 28);
                            var xEntry = new FatDirectoryEntry((FatFileSystem)mFileSystem, xParent, xFullPath, xName,
                                xSize, xFirstCluster, i, DirectoryEntryTypeEnum.Directory);
                            Global.mFileSystemDebugger.SendInternal(xEntry.mName + " <DIR> " + xEntry.mSize +
                                                                    " bytes : Attrib = " + xAttrib + ", Status = " +
                                                                    xStatus);
                            xResult.Add(xEntry);
                        }
                        else if (xTest == FatDirectoryEntryAttributeConsts.VolumeID)
                        {
                            Global.mFileSystemDebugger.SendInternal("<VOLUME ID> : Attrib = " + xAttrib +
                                                                    ", Status = " + xStatus);
                        }
                        else
                        {
                            Global.mFileSystemDebugger.SendInternal("<INVALID ENTRY> : Attrib = " + xAttrib +
                                                                    ", Status = " + xStatus);
                        }
    ...
    /// <returns>FatDirectoryEntry.</returns>
    /// <exception cref="Exception">
    ///     Thrown when trying to access VolumeId out of Root Directory / data size invalid / invalid
    ...
    public FatDirectoryEntry FindVolumeId()
    {
        ...
        for (uint i = 0; i < xData.Length; i = i + 32)
        {
            ...
            var xName = Encoding.ASCII.GetString(xData, (int)i, 11);
            xName = xName.TrimEnd();
            ...
            var xFirstCluster = xParent.mFirstClusterNum;

            Global.mFileSystemDebugger.SendInternal(
                $"VolumeID Found xName {xName} xFullPath {xFullPath} xSize {xSize} xFirstCluster {xFirstCluster}");

            xResult = new FatDirectoryEntry((FatFileSystem)mFileSystem, xParent, xFullPath, xName, xSize, xFirstCluster,
                i, DirectoryEntryTypeEnum.File);
            break;
        }
    ...
    /// <exception cref="OverflowException">Thrown when data lenght is greater then Int32.MaxValue.</exception>
    /// <exception cref="Exception">
    ///     Thrown when called on a directory other then root / data size invalid / invalid directory
    ///     entry type / memory error.
    /// </exception>
    /// <exception cref="ArgumentException">Thrown on memory error.</exception>
    /// <exception cref="DecoderFallbackException">Thrown on memory error.</exception>
    /// <exception cref="RankException">Thrown on fatal error (contact support).</exception>
    ...
    public FatDirectoryEntry CreateVolumeId(string name)
    {
        ...
        VolumeId.SetDirectoryEntryMetadataValue(FatDirectoryEntryMetadata.Attributes,
            FatDirectoryEntryAttributeConsts.VolumeID);
    ...
    /// <summary>
    ///     Tries to find an empty space for a directory entry and returns the offset to that space if successful, otherwise
    ///     throws an exception.
    /// </summary>
    ...
    private uint GetNextUnallocatedDirectoryEntry()
    {
        ...
        for (uint i = 0; i < xData.Length; i += 32)
        {
            var x1 = BitConverter.ToUInt32(xData, (int)i);
            var x2 = BitConverter.ToUInt32(xData, (int)i + 8);
            var x3 = BitConverter.ToUInt32(xData, (int)i + 16);
            var x4 = BitConverter.ToUInt32(xData, (int)i + 24);
    ...
    /// <summary>
    ///     Tries to find an empty space for the specified number of directory entries and returns an array of offsets to those
    ///     spaces if successful, otherwise throws an exception.
    /// </summary>
    ...
    /// <exception cref="OverflowException">Thrown when data lenght is greater then Int32.MaxValue.</exception>
    /// <exception cref="Exception">
    ///     Thrown when requested memory block of the size of aEntryCount not found / invalid directory
    ///     entry type.
    /// </exception>
    private uint[] GetNextUnallocatedDirectoryEntries(int aEntryCount)
    {
        ...
        var xData = GetDirectoryEntryData();
        var xCount = 0;
        var xEntries = new uint[aEntryCount];
    ...
    internal void SetDirectoryEntryMetadataValue(FatDirectoryEntryMetadata aEntryMetadata, byte aValue)
    {
        ...
        if (xData.Length > 0)
        {
            var xOffset = mEntryHeaderDataOffset + aEntryMetadata.DataOffset;
    ...
    internal void SetDirectoryEntryMetadataValue(FatDirectoryEntryMetadata aEntryMetadata, ushort aValue)
    {
        ...
        if (xData.Length > 0)
        {
            ...
            var offset = mEntryHeaderDataOffset + aEntryMetadata.DataOffset;
    ...
    ///     </list>
    /// </exception>
    internal void SetLongFilenameEntryMetadataValue(uint aEntryHeaderDataOffset,
        FatDirectoryEntryMetadata aEntryMetadata, uint aValue)
    {
        ...
        if (xData.Length > 0)
        {
            ...
            xValue.SetUInt32(0, aValue);
            var offset = aEntryHeaderDataOffset + aEntryMetadata.DataOffset;
    ...
    /// </exception>
    internal void SetLongFilenameEntryMetadataValue(uint aEntryHeaderDataOffset,
        FatDirectoryEntryMetadata aEntryMetadata, long aValue)
    {
    ...
    /// </exception>
    internal void SetLongFilenameEntryMetadataValue(uint aEntryHeaderDataOffset,
        FatDirectoryEntryMetadata aEntryMetadata, string aValue)
    {
    ...
    internal static string GetShortName(string aShortName)
    {
        ...
        for (var i = 0; i < aShortName.Length; i++)
        {
            if (aShortName[i] == '.')
            {
                i++;
    ...
    private long GetDirectoryEntrySize(byte[] DirectoryEntryData)
    {
        ...
        {
            var xAttrib = DirectoryEntryData[i + 11];
            var xStatus = DirectoryEntryData[i];
            ...
            switch (xTest)
            {
                // Normal file
                case 0:
                    var xSize = BitConverter.ToUInt32(DirectoryEntryData, (int)i + 28);
                    xResult += xSize;
                    break;
                ...
                case FatDirectoryEntryAttributeConsts.Directory:
                    ...
                    var xFirstCluster = (uint)((BitConverter.ToUInt16(DirectoryEntryData, (int)i + 20) << 16) |
                                               BitConverter.ToUInt16(DirectoryEntryData, (int)i + 26));
                    byte[] xDirData;
