...
[assembly:
    InternalsVisibleTo(
        "Cosmos.System.Tests, PublicKey=")]
...
internal class FatFileSystem : FileSystem
{
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="FatFileSystem" /> class, then format the partition into a new FAT
    ///     filesystem.
    /// </summary>
    /// <param name="aDevice">The partition.</param>
    /// <param name="aRootPath">The root path.</param>
    ...
    internal void Read(long aCluster, out byte[] aData)
    {
        ...
        {
            aData = NewBlockArray();
            var xSector = DataSector + (aCluster - RootCluster) * SectorsPerCluster;
    ...
    /// <exception cref="ArgumentOutOfRangeException">
    ///     Thrown when root directory address is smaller then root directory
    ///     address.
    /// </exception>
    /// <exception cref="ArgumentNullException">Thrown when filesystem is null.</exception>
    ...
    public override void Format(string aDriveFormat, bool aQuick)
    {
        ...
        if (FileSystemExists)
        {
            ...
            {
                Device.WriteBlock((ulong)ReservedSectorCount + FatSectorCount * fat, FatSectorCount,
                    ref emptyFat.memory);
            }
    ...
    private ulong GetFatSizeSectors()
    {
        ...
        var Numerator = TotalSectorCount - ReservedSectorCount + ReservedClusCnt * SectorsPerCluster;
        var Denominator = SectorsPerCluster * BytesPerSector / FatElementSize + NumberOfFATs;
    ...
    internal class Fat
    {
        ...
        public uint[] GetFatChain(uint aFirstEntry, long aDataSize = 0)
        {
            ...
            var xCurrentEntry = aFirstEntry;
            uint xValue;

            var xEntriesRequired = aDataSize / mFileSystem.BytesPerCluster;
            ...
            if (xEntriesRequired > 0)
            {
                ...
                {
                    var xNewClusters = xEntriesRequired - xReturn.Length;
                    for (var i = 0; i < xNewClusters; i++)
                    {
                        xCurrentEntry = GetNextUnallocatedFatEntry();
                        mFileSystem.Write(xCurrentEntry, new byte[mFileSystem.BytesPerCluster]);
                        var xLastFatEntry = xReturn[xReturn.Length - 1];
            ...
            var xChain = "";
            for (var i = 0; i < xReturn.Length; i++)
            {
                xChain += xReturn[i];
        ...
        public uint GetNextUnallocatedFatEntry()
        {
            ...
            var xTotalEntries = mFileSystem.FatSectorCount * mFileSystem.BytesPerSector / GetFatEntrySizeInBytes();
            for (var i = mFileSystem.RootCluster + 1; i < xTotalEntries; i++)
            {
        ...
        public void ClearFatEntry(ulong aEntryNumber) => SetFatEntry(aEntryNumber, 0);
        ...
        {
            var xEntrySize = GetFatEntrySizeInBytes();
            var xEntryOffset = aEntryNumber * xEntrySize;
        ...
        public void ClearAllFat()
        {
            ...
            var xFatTable = mFileSystem.NewBlockArray();
        ...
        internal void GetFatEntry(uint aEntryNumber, out uint aValue)
        {
            ...
            var xSector = xEntryOffset / mFileSystem.BytesPerSector;
            Global.mFileSystemDebugger.SendInternal("xSector = " + xSector);

            ReadFatSector(xSector, out var xData);
        ...
        internal void SetFatEntry(ulong aEntryNumber, byte[] aData, uint aOffset, uint aLength)
        {
            ...
            var xSector = xEntryOffset / mFileSystem.BytesPerSector;
            var xSectorOffset = xSector * mFileSystem.BytesPerSector - xEntryOffset;
