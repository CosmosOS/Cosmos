...
{
    private static readonly List<KeyboardBase> mKeyboardList = new();
    private static ScanMapBase mScanMap = new US_Standard();
    private static readonly Queue<KeyEvent> mQueuedKeys = new();
    ...
    /// <exception cref="IOException">An I/O error occurred.</exception>
    internal static void HandleFakeScanCode(byte aScancode, bool aReleased) => HandleScanCode(aScancode, aReleased);
    ...
    private static void HandleScanCode(byte aScanCode, bool aReleased)
    {
        ...
        else if (mScanMap.ScanCodeMatchesKey(key, ConsoleKeyEx.LCtrl) ||
                 mScanMap.ScanCodeMatchesKey(key, ConsoleKeyEx.RCtrl))
        {
            ControlPressed = !aReleased;
        }
        else if (mScanMap.ScanCodeMatchesKey(key, ConsoleKeyEx.LShift) ||
                 mScanMap.ScanCodeMatchesKey(key, ConsoleKeyEx.RShift))
        {
            ShiftPressed = !aReleased;
        }
        else if (mScanMap.ScanCodeMatchesKey(key, ConsoleKeyEx.LAlt) ||
                 mScanMap.ScanCodeMatchesKey(key, ConsoleKeyEx.RAlt))
        {
    ...
    {
        foreach (var keyboard in mKeyboardList)
        {
            keyboard.UpdateLeds();
        }
    }
    ...
    public static bool GetKey(byte aScancode, out KeyEvent keyInfo)
    {
        ...
        keyInfo = mScanMap.ConvertScanCode(aScancode, ControlPressed, ShiftPressed, AltPressed, NumLock, CapsLock,
            ScrollLock);
    ...
    public static ScanMapBase GetKeyLayout() => mScanMap;
