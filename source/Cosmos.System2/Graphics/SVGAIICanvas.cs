...
{
    private static readonly Mode _DefaultMode = new(1024, 768, ColorDepth.ColorDepth32);
    ...
    /// </summary>
    public override void Disable() => _xSVGADriver.Disable();
    ...
    public override void DrawPoint(Pen aPen, float aX, float aY) => throw
        //xSVGAIIDriver.
    ...
    public override void DrawFilledRectangle(Pen aPen, int aX_start, int aY_start, int aWidth, int aHeight)
    {
        ...
        for (var i = aY_start; i < aY_start + aHeight; i++)
        {
    ...
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    /// <exception cref="NotImplementedException">Thrown if VMWare SVGA 2 has no rectange copy capability</exception>
    public override void Clear(int aColor) => _xSVGADriver.Clear((uint)aColor);
    ...
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    /// <exception cref="NotImplementedException">Thrown if VMWare SVGA 2 has no rectange copy capability</exception>
    public override void Clear(Color aColor) => _xSVGADriver.Clear((uint)aColor.ToArgb());
    ...
    /// <param name="aX">A X coordinate.</param>
    /// <param name="aY">A Y coordinate.</param>
    public void SetCursor(bool aVisible, int aX, int aY) => _xSVGADriver.SetCursor(aVisible, (uint)aX, (uint)aY);

    /// <summary>
    ///     Create cursor.
    /// </summary>
    public void CreateCursor() => _xSVGADriver.DefineCursor();
    ...
    public override Color GetPointColor(int aX, int aY) =>
        Color.FromArgb((int)_xSVGADriver.GetPixel((uint)aX, (uint)aY));

    /// <summary>
    ///     Display screen
    /// </summary>
    public override void Display() => _xSVGADriver.DoubleBufferUpdate();
    ...
    public override void DrawImage(Image aImage, int aX, int aY)
    {
        ...
        {
            _xSVGADriver.VideoMemory.Copy(GetPointOffset(aX, aY + i) + (int)_xSVGADriver.FrameSize, aImage.rawData,
                i * xWidth, xWidth);
        }
    }
}
