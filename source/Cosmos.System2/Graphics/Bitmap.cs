...
public class Bitmap : Image
{
            ...
            if (colorDepth == ColorDepth.ColorDepth32)
            {
                rawData[i] =
                    BitConverter.ToInt32(
                        new[] { pixelData[i * 4], pixelData[i * 4 + 1], pixelData[i * 4 + 2], pixelData[i * 4 + 3] },
                        0);
            }
            else
            {
                rawData[i] =
                    BitConverter.ToInt32(new byte[] { 0, pixelData[i * 3], pixelData[i * 3 + 1], pixelData[i * 3 + 2] },
                        0);
            }
        }
    }
    ...
    public Bitmap(string path, ColorOrder colorOrder = ColorOrder.BGR) :
        base(0, 0, ColorDepth.ColorDepth32) //Call the image constructor with wrong values
    {
    ...
    private void CreateBitmap(Stream stream, ColorOrder colorOrder)
    {
        ...
        var fileSize = BitConverter.ToUInt32(_int, 0);

        stream.Position = 10;
        //read header - bytes 10 -> 14 is the offset of the bitmap image data
        stream.Read(_int, 0, 4);
        var pixelTableOffset = BitConverter.ToUInt32(_int, 0);

        //now reading size of BITMAPINFOHEADER should be 40 - bytes 14 -> 18
        stream.Read(_int, 0, 4);
        var infoHeaderSize = BitConverter.ToUInt32(_int, 0);
        if (infoHeaderSize != 40 && infoHeaderSize != 56 &&
            infoHeaderSize !=
            124) // 124 - is BITMAPV5INFOHEADER, 56 - is BITMAPV3INFOHEADER, where we ignore the additional values see https://web.archive.org/web/20150127132443/https://forums.adobe.com/message/3272950
        {
        ...
        stream.Read(_int, 0, 4);
        var imageWidth = BitConverter.ToUInt32(_int, 0);

        //now reading height of image in pixels - byte 22 -> 26
        stream.Read(_int, 0, 4);
        var imageHeight = BitConverter.ToUInt32(_int, 0);

        //now reading number of planes should be 1 - byte 26 -> 28
        stream.Read(_short, 0, 2);
        var planes = BitConverter.ToUInt16(_short, 0);
        if (planes != 1)
        {
        ...
        stream.Read(_short, 0, 2);
        var pixelSize = BitConverter.ToUInt16(_short, 0);
        ...
        stream.Read(_int, 0, 4);
        var compression = BitConverter.ToUInt32(_int, 0);
        ...
        stream.Read(_int, 0, 4);
        var totalImageSize = BitConverter.ToUInt32(_int, 0);
        if (totalImageSize == 0)
        {
            totalImageSize =
                (uint)(((imageWidth * pixelSize + 31) & ~31) >> 3) *
                imageHeight; // Look at the link above for the explanation
        ...
        //Calculate padding
        int paddingPerRow;
        var pureImageSize = (int)(imageWidth * imageHeight * pixelSize / 8);
        if (totalImageSize != 0)
        {
        ...
        var position = 0;
        var pixelData = new byte[pureImageSize];
        ...
        for (var y = 0; y < imageHeight; y++)
        {
            for (var x = 0; x < imageWidth; x++)
            {
                ...
                rawData[x + (imageHeight - (y + 1)) * imageWidth] =
                    BitConverter.ToInt32(pixel, 0); //This bits should be A, R, G, B but order is switched
            }
    ...
    {
        //Calculate padding
        var padding = 4 - (int)Width * (int)Depth % 32 / 8;
        if (padding == 4)
        {
            padding = 0;
        }

        var file = new byte[54 /*header*/ + Width * Height * (uint)Depth / 8 + padding * Height];
        //Writes all bytes at the end into the stream, rather than a few every time

        var position = 0;
        //Set signature
        var data = BitConverter.GetBytes(0x4D42);
        Array.Copy(data, 0, file, position, 2);
        position += 2;

        //Write apporiximate file size
        data = BitConverter.GetBytes(
            54 /*header*/ + Width * Height * (uint)Depth / 8 /*assume that it is full bytes */);
        ...
        //Copy image data
        position = (int)offset;
        var byteNum = (int)Depth / 8;
        var imageData = new byte[Width * Height * byteNum + padding * Height];
        var imageDataPoint = 0;
        var cOffset = 4 - (int)Depth / 8;
