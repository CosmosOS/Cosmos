...
public class VBECanvas : Canvas
{
    ...
    public VBECanvas(Mode aMode)
    {
        Global.mDebugger.SendInternal(
            $"Creating new VBEScreen() with mode {aMode.Columns}x{aMode.Rows}x{(uint)aMode.ColorDepth}");
        ...
        if (VBE.IsAvailable())
        {
            ...
            Global.mDebugger.SendInternal(
                $"Detected VBE VESA with {aMode.Columns}x{aMode.Rows}x{(uint)aMode.ColorDepth}");
        }
    ...
    /// </summary>
    public override void Disable() => _VBEDriver.DisableDisplay();
    ...
    ///     Display screen
    /// </summary>
    public override void Display() => _VBEDriver.Swap();
    ...
    {
        var offset = (uint)GetPointOffset(aX, aY);
    ...
    private void SetMode(Mode aMode)
    {
        ...
        var xres = (ushort)Mode.Columns;
        var yres = (ushort)Mode.Rows;
        var bpp = (ushort)Mode.ColorDepth;
    ...
    public override void DrawPoint(Pen aPen, int aX, int aY)
    {
        ...
        switch (Mode.ColorDepth)
        {
            ...
            case ColorDepth.ColorDepth24:
                ...
                break;
            default:
                var errorMsg = "DrawPoint() with ColorDepth " + (int)Mode.ColorDepth + " not yet supported";
    ...
    public override void DrawImage(Image aImage, int aX, int aY)
    {
        ...
        var xOffset = GetPointOffset(aX, aY);
        var xScreenWidthInPixel = Mode.Columns;
