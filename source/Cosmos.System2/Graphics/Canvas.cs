...
public abstract class Canvas
{
    ...
    public virtual void Clear(Color color)
    {
        ...
        for (var x = 0; x < Mode.Rows; x++)
        {
            for (var y = 0; y < Mode.Columns; y++)
            {
                DrawPoint(pen, x, y);
            }
        }
    ...
    {
        var xBytePerPixel = (int)Mode.ColorDepth / 8;
        var stride = (int)Mode.ColorDepth / 8;
        var pitch = Mode.Columns * xBytePerPixel;
    ...
    public void DrawLine(Pen pen, float x1, float y1, float x2, float y2) => throw new NotImplementedException();
    ...
    /// <exception cref="ArgumentOutOfRangeException">Thrown if coorinates invalid.</exception>
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    public virtual void DrawCircle(Pen pen, Point point, int radius) => DrawCircle(pen, point.X, point.Y, radius);
    ...
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    /// <exception cref="OverflowException">Thrown if lines length are invalid.</exception>
    public virtual void DrawSquare(Pen pen, Point point, int size) => DrawRectangle(pen, point.X, point.Y, size, size);
    ...
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    /// <exception cref="OverflowException">Thrown if lines length are invalid.</exception>
    public virtual void DrawSquare(Pen pen, int x, int y, int size) => DrawRectangle(pen, x, y, size, size);
    ...
    {
        for (var _x = 0; _x < image.Width; _x++)
        {
            for (var _y = 0; _y < image.Height; _y++)
            {
    ...
    {
        var pixels = image.rawData;
        var w1 = (int)image.Width;
        var h1 = (int)image.Height;
        var temp = new int[newWidth * newHeight];
        var x_ratio = (w1 << 16) / newWidth + 1;
        var y_ratio = (h1 << 16) / newHeight + 1;
        int x2, y2;
    ...
    {
        var pixels = scaleImage(image, w, h);
    ...
    /// <exception cref="Exception">Thrown on memory access violation.</exception>
    /// <exception cref="ArgumentOutOfRangeException">Thrown on fatal error.</exception>
    public void DrawImageAlpha(Image image, Point point) => DrawImageAlpha(image, point.X, point.Y);
    ...
    public void DrawChar(char c, Font aFont, Pen pen, Point point) => DrawChar(c, aFont, pen, point.X, point.Y);
    ...
    {
        var p = aFont.Height * (byte)c;

        for (var cy = 0; cy < aFont.Height; cy++)
        {
    ...
    protected void ThrowIfCoordNotValid(Point point) => ThrowIfCoordNotValid(point.X, point.Y);
    ...
    protected void TrimLine(ref int x1, ref int y1, ref int x2, ref int y2)
    {
        ...
        var m = (y2_out - y1_out) / (x2_out - x1_out);
        var c = y1_out - m * x1_out;
    ...
    {
        var R = (byte)((to.R * alpha + from.R * (255 - alpha)) >> 8);
        var G = (byte)((to.G * alpha + from.G * (255 - alpha)) >> 8);
        var B = (byte)((to.B * alpha + from.B * (255 - alpha)) >> 8);
